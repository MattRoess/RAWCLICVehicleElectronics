...
```bash
git status --short        # expect docs/06_PCB_MODEL_DESIGN.md, possibly untracked
```

`docs/06_PCB_MODEL_DESIGN.md` was written and revised but **never committed**. It
...
Commits on **2026-08-10**, newest last:
    ...
    (uncommitted)  docs/06_PCB_MODEL_DESIGN.md, incl. the 800V research
...
**Read `docs/06_PCB_MODEL_DESIGN.md` first.** The single most important finding in
...
docs/          cross-cutting analysis, tracked
  HANDOVER_2026-08-09.md                                 this file
  05_ADAS_SENSOR_ADOPTION_REPORT.md    the sensor evidence base
...
2. **`Wiring/IMPLEMENTATION_GUIDE.md`** — the hardware-tier axis: data flow,
   ...
   rollback switch.
3. **`docs/05_ADAS_SENSOR_ADOPTION_REPORT.md`** — the sensor
...
Read-only. Full detail in `docs/07_STATIC_MODELS_DIAGNOSTIC.md`.
...
`docs/11_BRAND_ORIGIN_DESIGN.md`. **Not implemented.**
...
| 5 | **Option B** — front-corner and cabin/child-presence cameras as new components. Documented in `04_SENSOR_MODEL_DESIGN.md` S10.5, not built. | structural |
